...
```bash
alias chunnel-install='curl -fL https://github.com/griigoriiy/chunnel/releases/latest/download/charles-tunnel.apk -o "$HOME/Downloads/charles-tunnel.apk" && adb install -r "$HOME/Downloads/charles-tunnel.apk" && (adb shell pm grant com.mobileapp.charlestunnel android.permission.POST_NOTIFICATIONS 2>/dev/null || true)'
